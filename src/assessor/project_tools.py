...
def search_code_content(project_path: Path, pattern: str, file_extension: str = ".py") -> list[dict[str, str]]:
    ...
    files = [f for f in files if not any(part.startswith('.') for part in f.parts) and '__pycache__' not in str(f)]
    ...
    for file in files[:100]:
        try:
            ...
            if pattern.lower() in content.lower():
                ...
                matching_lines = [(i, line) for i, line in enumerate(lines) if pattern.lower() in line.lower()]
                
                if matching_lines:
                    results.append({
                        "file": str(file.relative_to(project_path)),
                        "matches": len(matching_lines),
                        "sample_lines": [f"Line {i+1}: {line.strip()}" for i, line in matching_lines[:3]]
                    })
        except Exception:
            pass
...
def read_file_content(project_path: Path, relative_path: str, max_lines: int = 200) -> str:
